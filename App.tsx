...
Repositories to analyze: ${JSON.stringify(repoContext)}`,
  ...
  const filteredRepos = useMemo(() => {
    ...
    if (isSmartFilterMode && aiFilteredIds === null && !isAiFiltering) return result;
  ...
  }, [activeTab, repos, savedRepos, searchQuery, aiFilteredIds, isSmartFilterMode, isAiFiltering]);
            ...
            <form onSubmit={handleSearchSubmit} className="relative flex items-center">
              <MotionDiv 
                animate={{ 
                  ...
                  borderColor: isSmartFilterMode ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.05)',
                  backgroundColor: isSmartFilterMode ? 'rgba(99,102,241,0.05)' : 'rgba(255,255,255,0.05)'
                }}
              ...
              >
                ...
                <input 
                  type="text"
                  placeholder={isSmartFilterMode ? "Ask AI (e.g. 'performance tools')..." : "Find a repository..."}
                  value={searchQuery}
              ...
              >
                <Wand2 size={18} className={isSmartFilterMode ? "animate-pulse" : ""} />
                <span className={`text-xs font-bold mono uppercase tracking-tight overflow-hidden transition-all duration-300 ${isSmartFilterMode ? 'max-w-[80px] opacity-100' : 'max-w-0 opacity-0'}`}>
                  Smart
                </span>
              </button>
            </form>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="popLayout">
            {isLoading && page === 1 ? (
              ...
              >
                <div className="w-20 h-20 rounded-full bg-white/[0.02] border border-white/5 flex items-center justify-center text-slate-600 mb-6">
                  {searchQuery || aiFilteredIds ? <Filter size={32} /> : activeTab === 'saved' ? <Bookmark size={32} /> : <Search size={32} />}
                </div>
                <h3 className="text-xl font-semibold text-slate-200 mb-2">
                  {aiFilteredIds ? "AI found no matches" : searchQuery ? "No results found" : activeTab === 'saved' ? "Your library is empty" : "Something went wrong"}
                </h3>
            ...
            {isAiFiltering && (
              Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={`ai-loading-${i}`} />)
            )}
          </AnimatePresence>
        </div>
